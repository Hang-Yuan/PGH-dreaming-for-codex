...
version: 2.1
status: active
description: Codex 节律表、原生自动化任务验收与梦境事务闸。
---
...
| 时机 | 跑什么 | 触发者 |
|---|---|---|
| 每次会话开头 | 启动序列 → `week-sync` | Codex 运行时 |
| 节点在对话中闭合 | `close-node` | 主代理|
| 日界线 + 30 分钟 | `daily-dream` | 唯一一条 Codex 原生自动化任务 |
...
- 发布包不含系统级调度安装器或无头包装器。
- 现役文档只声明 Codex 原生自动化任务。
- 旧 `.pgh` 排程收据不参与时区、日界线或任务状态解析。
- 每日自动化任务数量为一。
- 周级 / 季度技能不拥有排程。
- 自动化任务定义与梦境地面收据分开核验。
