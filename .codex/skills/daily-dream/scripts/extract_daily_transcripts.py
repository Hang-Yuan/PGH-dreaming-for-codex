...
def main() -> int:
    args = parse_args()
    tz_name, tz_src = resolve_timezone(args.timezone)
    try:
    ...
    boundary, src = resolve_boundary_hour(args.boundary_hour)
    # 两个来源都打到 stderr：窗口错位不会报错，只会安静地抽错一段，故必须让运行时看得
    # 见「这一趟按哪个日界线、哪个时区切的」。
    print(f"boundary hour = {boundary:02d}:00 ({src})", file=sys.stderr)
    print(f"timezone = {tz_name} ({tz_src})", file=sys.stderr)
    start_local = datetime.combine(target, time(hour=boundary), tzinfo=tz)
    ...
    output_dir = args.output_dir or Path("/tmp") / "daily-dream" / target.isoformat()
    write_bundle(output_dir, target, tz_name, start_local, end_local, file_infos, messages)
    summary = {
        "logical_date": target.isoformat(),
