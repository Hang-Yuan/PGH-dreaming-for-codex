...
## v6.2.3 · 原生定时任务闭环与旧收据退役

- 初始化任务明确落到 Codex 应用“定时任务”页，字段收敛为原生 `cron`、`rrule`、`projectId`、`executionEnvironment=local`、模型与推理强度。
- 转写抽取器只从已安装 `AGENTS.md §时间感知` 读取 IANA 时区和日界线；旧 `.pgh` OS 排程收据不再参与窗口计算。
- 旧路由审计扩展到 skill 脚本，并新增 OS 排程制品规则，防止旧收据或安装器依赖重新进入现役链。
